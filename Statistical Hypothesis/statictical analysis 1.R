...
# Select the variables we want to include in our table
vars_to_analyze <- c("lipids1", "lipids2", "lipids3", "lipids4")

# Apply the function to each variable and combine results
stats_list <- lapply(data_for_analysis[vars_to_analyze], calculate_stats)

# Combine all results into one table
custom_stats_table <- do.call(rbind, stats_list)

# Add variable names as a column
custom_stats_table <- cbind(Variable = rownames(custom_stats_table), custom_stats_table)
rownames(custom_stats_table) <- NULL

# Format the table nicely
custom_stats_table <- as.data.frame(custom_stats_table)

# Print the table
print(custom_stats_table)

# Optionally, save to CSV
write.csv(custom_stats_table, "custom_descriptive_stats.csv", row.names = FALSE)
